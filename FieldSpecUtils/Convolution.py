...
def GeoEYE(spectra, Bands):
    ...
    for band_file in os.scandir(bands_Dir):
        file_name = Path(band_file).stem
        convolution_process = pd.read_csv(band_file, index_col=0, header=0)
        Blue = (np.trapz((convolution_process.iloc[0:650, 0]), axis = 0)) / (np.trapz((Bands.iloc[0:650, 0]), axis = 0))
        Green = (np.trapz((convolution_process.iloc[0:650, 1]), axis = 0)) / (np.trapz((Bands.iloc[0:650, 1]), axis = 0))
        Red = (np.trapz((convolution_process.iloc[0:650, 2]), axis = 0)) / (np.trapz((Bands.iloc[0:650, 2]), axis = 0))
        NIR = (np.trapz((convolution_process.iloc[0:650, 3]), axis = 0)) / (np.trapz((Bands.iloc[0:650, 3]), axis = 0))
